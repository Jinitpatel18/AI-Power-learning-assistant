    ...
    return (
        <div className='flex flex-col items-center justify-center py-16 px-6 text-center bg-linear-to-br from-slate-50/50 to-white border-2 border-dashed border-slate-200 rounded-3xl'>
            ...
            {buttonText && onActionClick && (
                ...
                className='group relative inline-flex items-center gap-2 px-6 h-11 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 focus:outline-none focus:right-4 focus:ring-emerald-500/20 overflow-hidden'>
                    <span className='relative z-10 flex  items-center gap-2'>
                        <Plus className='w-4 h-4' strokeWidth={2.5} />
                        {buttonText}
                    </span>
                    <div className='absolute inset-0 bg-linear-to-r from-white/0 viw-white/20 to-white/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700' />
                </button>
            )}
        </div>
    )

...
const FlashcardManager = ({ documentId }) => {
    ...
    const handleGenerateFlashcards = async () => {
        ...
        try {
            ...
            toast.success('Flashcards generate successfully.')
            fetchFlashcardSets();
        } catch (error) {
            ...
            <div className='space-y-6'>
                ...
                <div className='flex flex-col items-center space-y-8'>
                    ...
                    <div className='flex items-center gap-6'>
                        <button onClick={handlePreviewCard}
                        ...
                        <button
                            ...
                            disabled={selectedSet.cards.length <= 1}
                            className='group flex items-center gap-2 px-5 h-11 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-sm rounded-xl duration-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-slate-100'>
                            Next
                            <ChevronRight className='w-4 h-4 group-hover:translate-x-0.5 transition-tranform duration-200' strokeWidth={2} />
                        </button>
                    </div>
                </div>
            </div>
        ...
        if (flashcardSets.length === 0) {
            return (
                <div className='flex flex-col items-center justify-center py-16 px-6'>
                    <div className='inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-linear-o-br from-emerald-100 to-teal-100 mb-6'>
                    ...
                    <p className='text-sm text-slate-500 mb-8 text-center max-w-sm'>
                        Generate flashcards from your document to star learning and
                        reinforce your knowledge.
                    </p>
                    <button className='group inline-flex items-center gap-2 px-6 h-12 bg-linear-to-br from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100'
                        onClick={handleGenerateFlashcards}
                        disabled={generating}>
                        {generating ? (
                            <>
                                <div className='w-4 h-4 border-2 border-white/30 border-t-white rounded-all animate-spin' />
                                Generating....
                            </>
                        ) : (
                            <>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                    {flashcardSets.map((set) => (
                        <div key={set._id}
                            ...
                            className="group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-emerald-300 rounded-xl p-6 cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/10">
                            {/* Delete Button */}
                            <button

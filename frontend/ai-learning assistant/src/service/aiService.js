...
const generateQuiz = async (documentId, options) => {
    try {
        const response = await axiosInstance.post(API_PATHS.AI.GENERATE_QUIZ, { documentId })
        return response.data
    } catch (error) {
        throw error.response?.data || { message: 'Failed to generate quiz' }
    }
...
// const explainConcept = async( documentId ) => {
//     try {
...
const getChatHistory = async(documentId) => {
    try {
        ...
        throw error.response?.data || { message: 'Failed to getChatHistory'}
    }
}
